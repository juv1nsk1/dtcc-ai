...
def main(file_path=os.path.join("data", "processed.csv")):
    ...
    param_grid = {
        'n_estimators': [50, 100, 150, 200, 250, 300],  # Number of boosting iterations
        'learning_rate': [0.01, 0.1, 0.5],   # Learning rate for boosting
        'max_depth': [3, 5, 7, 9],            # Maximum depth of trees
        'subsample': [0.8, 1.0],        # Subsampling ratio of the training data
        'colsample_bytree': [0.8, 1.0]  # Subsampling ratio of features per tree
    }

    # Create a LightGBM classifier
    lgbm = LGBMClassifier(objective='binary', random_state=0, verbose = -1)
